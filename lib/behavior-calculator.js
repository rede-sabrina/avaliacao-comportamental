...
export function generateBehaviorResultHtml(result, candidate) {
  ...
  if (dominantProfiles.length === 0) {
    ...
    dominantHtml = dominantProfiles.map((dim, idx) => {
      const scoreData = getScoreForDimension(dim);
  ...
  // Mapeamento para encontrar score pela dimensão (com/sem acentos)
  const dimKeyMap = {
    'DOMINÂNCIA': 'dominancia',
    'INFLUÊNCIA': 'influencia',
    'ESTABILIDADE': 'estabilidade',
  ...
  function getScoreForDimension(dim) {
    const key = dimKeyMap[dim] || dim.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
