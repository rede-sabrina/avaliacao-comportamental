...
// Mapeamento para encontrar score pela dimensão (com/sem acentos) - MODULE LEVEL para evitar TDZ no minificador
const DIM_KEY_MAP = {
  'DOMINÂNCIA': 'dominancia',
  'INFLUÊNCIA': 'influencia',
  'ESTABILIDADE': 'estabilidade',
...
function getScoreForDimension(scores, dim) {
  const key = DIM_KEY_MAP[dim] || dim.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
...
export function generateBehaviorResultHtml(result, candidate) {
  ...
  if (dominantProfiles.length === 0) {
    ...
    dominantHtml = dominantProfiles.map((dim, idx) => {
      const scoreData = getScoreForDimension(scores, dim);
